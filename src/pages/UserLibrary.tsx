...
const UserLibrary: React.FC = () => {
  ...
  const { loadPreset, token } = useSimulationStore();
  ...
  if (isLoading) return <div className={styles.page}><div className={styles.header}>Loading...</div></div>;
  if (!profile) return <div className={styles.page}><div className={styles.header}>User not found.</div></div>;
  ...
  return (
    <div className={styles.page}>
      <main className={styles.content}>
        <div className={styles.header} style={{ marginBottom: '40px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '10px', justifyContent: 'center' }}>
             ...
             <div style={{ textAlign: 'left' }}>
                <h1 style={{ margin: 0, fontSize: '32px' }}>{profile.username}'s Collection</h1>
                <div style={{ display: 'flex', gap: '20px', color: '#666', marginTop: '5px' }}>
        ...
        {presets.length === 0 ? (
          <div className={styles.loginMessage}>No presets found for this user.</div>
        ) : (
            ...
            {presets.map((preset) => (
              <div key={preset.id} className={styles.card}>
                ...
                <div className={styles.paramsPreview}>
                  {Object.entries(preset.parameters).slice(0, 3).map(([k, v]) => (
                    <div key={k} className={styles.paramItem}>
                      <span className={styles.paramKey}>{k}:</span>
                      <span className={styles.paramVal}>{(v as number).toFixed(2)}</span>
                    </div>
                  ))}
                  {Object.keys(preset.parameters).length > 3 && <span>...</span>}
                </div>

                <div className={styles.cardFooter}>
                  <div className={styles.presetMeta}>
                    {!preset.isPublic && <span className={styles.privateBadge}>Private</span>}
                    <span>{new Date(preset.createdAt).toLocaleDateString()}</span>
                  </div>
                  <button onClick={() => handleLoad(preset)} className={styles.loadButton}>
                    Load Preset
                  </button>
                </div>
              </div>
